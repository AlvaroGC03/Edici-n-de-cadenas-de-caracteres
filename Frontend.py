...
def explorador(archivos):
    ...
    ttk.Label(root, text="Ingrese el tamaño de la ventana").place(x=450, y=350) 
    ttk.Entry(root, textvariable=ventana).place(x=450, y=400) # etiqueta Solicita los chunks
    ttk.Label(root, text="Ingrese el tamaño del recorrido").place(x=450, y=450) 
    ttk.Entry(root, textvariable=recorrido).place(x=450, y=500) # etiqueta Solicita los chunks
    ttk.Button(root, text="Continuar", command=lambda: menu2(dataframe, ventana.get(), recorrido.get())).place(x=900, y=600) #Este boton continua el programa hacia el menu2
...
def menu2(dataframe, ventana, recorrido):
    ...
    ttk.Label(root, text="El archivo fue leido correctamente").place(x=410, y=15) #Mensaje principal de la ventana
    ttk.Label(root, text="Deseas Continuar?").place(x=460, y=80) #Etiqueta de continuar
    ttk.Button(root, text="Si", command=lambda: menu3(dataframe, ventana, recorrido)).place(x=470, y=200) #Boton que lleva al menu3 donde se imprimen los chunks
...
def menu3(dataframe, ventana, recorrido):
    ...
    ttk.Label(root, text="Los Chunks que se han genrado son:").place(x=435, y=65) #Mensaje de los chunks que se generaron
    ttk.Button(root, text="Combinar todo", command=lambda: menu4(ListaChunks, cadena)).place(x=750, y=650) #Boton para menu4 
    ttk.Button(root, text="Combinar por Rango", command=lambda: menu5(ListaChunks, cadena)).place(x=850, y=650) #Boton para menu5
...
def menu4(ListaChunks, cadena): #Combinar todo
    for widget in root.winfo_children():
        widget.destroy()
    ttk.Label(root, text="Estas son todas las combinaciones posibles:").place(x=435, y=65) #Etiqueta pricipal mensaje
...
def menu5(ListaChunks, cadena): #Combinar por Rango
    ...
    ttk.Entry(root, textvariable=rango_inicio).pack()
    ttk.Entry(root, textvariable=rango_fin).pack()
    ttk.Button(root, text="Continuar", command=lambda: Mostrar_rangos(ListaChunks, cadena, rango_inicio, rango_fin)).pack()
